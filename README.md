...
| assessment_tool_powershell | A Boolean indicating whether or not the tool is written in PowerShell; if it is then we will install a PowerShell system package.  On Kali Linux the package is installed via the package manager, while on other platforms a suitable package is downloaded from [PowerShell/PowerShell](https://github.com/PowerShell/PowerShell). | `false` | No |
| assessment_tool_powershell_version | A string indicating the version of PowerShell to install.  This variable is only read if PowerShell is being installed via a package downloaded from [PowerShell/PowerShell](https://github.com/PowerShell/PowerShell). | `7.6.2` | No |
| assessment_tool_python | A Boolean that *can* indicate that Python is to be installed.  If either assessment_tool_pip_packages or assessment_tool_pip_requirements_file is defined, or if either assessment_tool_python2 or assessment_tool_python_install_development_dependencies is `true`, then Python will be installed anyway and a Python virtual environment created, but this variable is occasionally useful, e.g., when installing a tool that lacks any Python packaging and has no dependencies. | `false` | No |
